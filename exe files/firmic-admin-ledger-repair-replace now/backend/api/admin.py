...
def backfill_missing_hookup_fees(
    ...
    """
    Add the one-time $49 setup fee to existing non-terminated companies
    only when that lifetime charge is missing.
    """
    companies = (
        db.query(Company)
    ...
    try:
        for company in companies:
            entry, was_created = ensure_company_hookup_fee(
                db,
                company,
            )
