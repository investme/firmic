...
export function CompanyHealthRing({
  ...
  const status =
    safe >= 85 ? "Excellent" :
    safe >= 70 ? "Healthy" :
    safe >= 50 ? "Needs attention" : "At risk";
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h3 className="mt-1 text-xl font-black text-slate-900">Company Health</h3>
        </div>
      ...
      <div className="mt-6 flex flex-col items-center">
        ...
        {reviewedLabel && <p className="mt-1 text-xs text-slate-400">{reviewedLabel}</p>}
      </div>
    </section>
  );
}
