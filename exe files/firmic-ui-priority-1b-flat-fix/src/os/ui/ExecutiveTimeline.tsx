...
export function ExecutiveTimeline({ items }: { items: ExecutiveTimelineItem[] }) {
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="mb-5 flex items-center justify-between gap-4">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-violet-600">Today</p>
          <h3 className="mt-1 text-xl font-black text-slate-900">Executive Timeline</h3>
        </div>
      ...
      <div className="space-y-5">
        {items.map((item, index) => (
          <div key={item.id} className="group relative pl-10">
            ...
            <span className={`absolute left-0 top-1 flex h-7 w-7 items-center justify-center rounded-full ring-4 ${toneMap[item.tone || "slate"]}`}>
              <span className="h-2 w-2 rounded-full bg-current" />
            </span>
            <div className="rounded-2xl px-3 py-2 transition hover:bg-violet-50/40">
